...
def conv(index, in_channels, out_channels, kernel_size, stride=2, padding=1, batch_norm=True, relu=False):
...
class DTN(object):
    ...
    """
    def __init__(self, mode='train', conv_dim, learning_rate=0.0003):
        self.mode = mode
    ...
    def content_extractor(self, images, reuse=False):
        ...
        if images.get_shape()[3] == 1:
            ...
            images = transforms.ToPILImage(mode=3)(images)
    ...
    def discriminator(self, images, reuse=False):
        ...
        net = nn.Sequential()
        net.add_module(conv(1, 3, self.conv_dim, 4, batch_norm=False))
        net.add_module(conv(2, self.conv_dim, self.conv_dim * 2, 4))
        net.add_module(conv(3, self.conv_dim *2, self.conv_dim * 4, 4))
        net.add_module(Flatten(self.conv_dim))
        net.add_module(nn.Linear(self.conv_dim*4*4*4, 1))
        net.add_module(nn.Dropout(0.5))
